#!/usr/bin/env python3
"""Generate responses for one already-downloaded mldsa-native ACVP session."""
...
from pqc_acvp.mldsa_runner import run_mldsa_file
...
def main() -> int:
    ...
    for request in requests:
        ...
        summary = run_mldsa_file(request, response, backend, backend.metadata)
        ...
        print(f"{summary.status}: mode={summary.mode} seen={summary.tests_seen} produced={summary.tests_produced}")

    passed = (
        {item["mode"] for item in vectors} == {"keyGen", "sigGen", "sigVer"}
        and all(item["status"] == "generated" for item in vectors)
    )
    session_summary = {
        "schemaVersion": 1,
    ...
    (session / "mldsa-native-iut-summary.json").write_text(
